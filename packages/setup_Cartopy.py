...
# GEOS
try:
    geos_version = b'3.11.0' # subprocess.check_output(['geos-config', '--version'])
...
def find_proj_version_by_program(conda=None):
    ...
    try:
        proj_version = b'8.0.1' # subprocess.check_output([proj],
                                              # stderr=subprocess.STDOUT)
        # proj_version = proj_version.split()[1].split(b'.')
        proj_version = proj_version.split(b'.')
...
if conda is not None and conda in sys.prefix:
    ...
    try:
        proj_version = b'8.0.1' # subprocess.check_output(['pkg-config', '--modversion',
                               #                 'proj'],
                               #                stderr=subprocess.STDOUT)
