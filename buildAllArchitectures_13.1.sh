...
# script to build with Xcode 13.1 (for iOS 14 devices).

# No spaces in Xcode name:
# sudo xcode-select -s /Applications/Xcode_13.1.app/
# setenv PATH /Users/holzschu/.cargo/bin:/Applications/Xcode_13.1.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/:/Applications/Xcode_13.1.app/Contents/Developer/usr/bin/:/usr/X11R6/bin:/usr/X11R6/bin:/usr/local/bin:/Users/holzschu/bin:/usr/bin:/bin:/usr/sbin:/sbin:/Library/TeX/texbin/
...
PLATFORM=iphoneos NPY_BLAS_ORDER="openblas" NPY_LAPACK_ORDER="openblas" MATHLIB="-lm" python3.9 setup.py build >> $PREFIX/make_ios.log 2>&1
	...
	# Making a big scipy library to load many modules (67 out of 95):
	pushd build/temp.macosx-${OSX_VERSION}-x86_64-3.9  >> $PREFIX/make_ios.log 2>&1
	clang -v -undefined error -dynamiclib \
		-arch arm64 -miphoneos-version-min=14.0 \
		-isysroot $IOS_SDKROOT \
		-lz -lm -lc++ \
