...
echo '{"url": "a-Shell/matplotlib-3.5.2", "dir_info": {}}' > /tmp/mpl.json
...
if [ -e "with_scipy/Library" ];then
	...
	find with_scipy/Library/lib/python3.11/site-packages/pulp/solverdir/cbc -type f -name cbc -delete
	# and re-create a fake binary:
	touch with_scipy/Library/lib/python3.11/site-packages/pulp/solverdir/cbc/osx/64/cbc
	#
	rm -f with_scipy/Library/lib/libpython3.11.dylib
	rm -f with_scipy/Library/bin/python3.11
	rm -f with_scipy/Library/bin/python3
	rm -rf with_scipy/Library/lib/python3.11/lib-dynload
	# Create fake binaries for pip
	touch with_scipy/Library/bin/python3
	touch with_scipy/Library/bin/python3.11
...
# Same, but inside Library_mini for a-Shell mini:
if [ -e "Library_mini" ];then
	find Library_mini -name __pycache__ -exec rm -rf {} \; >& find.log
	find Library_mini -name \*.pyc -delete
	find Library_mini -name \*.so -delete
	find Library_mini -name \*.a -delete
	find Library_mini -name \*.dylib -delete
	# Also remove MS Windows executables:
	find Library_mini -name \*.exe -delete
	find Library_mini -name \*.dll -delete
	rm -f Library_mini/lib/libpython3.11.dylib
	rm -f Library_mini/bin/python3.11
	rm -f Library_mini/bin/python3
	rm -rf Library_mini/lib/python3.11/lib-dynload
	# Create fake binaries for pip
	touch Library_mini/bin/python3
	touch Library_mini/bin/python3.11

# change direct_url.json files to have a more meaningful URL:
APP=$(basename `dirname $PWD`)
find Library_mini -type f -name direct_url.json -exec sed -i bak  "s/file:.*packages/${APP}/g" {} \; -print
find Library_mini -type f -name direct_url.jsonbak -delete
cp $PREFIX/build/lib.darwin-arm64-3.11/_sysconfigdata__darwin_darwin.py $PREFIX/Library_mini/lib/python3.11/_sysconfigdata__darwin_darwin.py
fi
